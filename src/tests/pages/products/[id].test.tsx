...
describe('Product page', () => {
  ...
  test('Adding item to cart should update the counters in header', () => {
    ...
    expect(wishlistCounter.textContent).toEqual('0')
    expect(shoppingBagCounter.textContent).toEqual('0')

    userEvent.click(wishlistButton)
    expect(wishlistCounter.textContent).toEqual('1')
    expect(shoppingBagCounter.textContent).toEqual('0')
    userEvent.click(shoppingBagButton)
    expect(wishlistCounter.textContent).toEqual('1')
    expect(shoppingBagCounter.textContent).toEqual('1')

    testStore.dispatch(setProduct(mockedProducts[1]))

    userEvent.click(shoppingBagButton)
    expect(wishlistCounter.textContent).toEqual('1')
    expect(shoppingBagCounter.textContent).toEqual('2')
    userEvent.click(wishlistButton)
    expect(wishlistCounter.textContent).toEqual('2')
    expect(shoppingBagCounter.textContent).toEqual('2')
  })
})
